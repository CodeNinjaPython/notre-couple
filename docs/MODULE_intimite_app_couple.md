...
> Contexte : usage **perso, un seul couple, deux adultes consentants**, FR, gratuit. J'ai donc retiré ce qui relève du « produit à vendre » (monétisation, multilingue, premium) et gardé ce qui sert *votre* usage.
...
> Si tu veux, je te génère le `.sql` complet de ces tables + RLS, dans le même format que `schema_couple_tracker.sql`.
